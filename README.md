...
GitInit: (Owner Repo) (NewBranch) (BranchName) -> effects
	With no args, pulls code from remote if .git subdirectory is present or assumes you would like to create a new shared repository out of the current directory, otherwise clones a remote repo specified by owner and repo name,
		optionally taking a name for created branch, and the branch to be cloned
